...
const Autorization = (props) => {
  ...
  const onFinish = (values) => {
    if (values.username !== props.users.name) {
      form.setFields([
        { errors: [`Неверное имя пользователя`], name: 'username' },
      ]);
    ...
    <div className={s.containerAuto}>
      ...
      <div className={s.formAuto}>
        <div className={s.titleAuto}>
          Авторизация
          <br />
          <span>(Захар, 1234)</span>
        </div>
        <Form form={form} name="autor" onFinish={onFinish} autoComplete="off">
          <Form.Item
            name="username"
          ...
          <Form.Item name="password" rules={[{ validator: validatorPass }]}>
            <div>
          ...
          <Form.Item name="but" style={{ marginBottom: 20 }}>
            <button className={s.butAut} type={'submit'}>
              Вход
            </button>
          </Form.Item>
        </Form>
        <a href="/" className={s.linkAuto}>
          Забыли свой пароль?
        </a>
