...
const Document = (props) => {
  ...
  const columns = [
    {
      title: () => (
        <div style={{ color: `var( --primary-blue)` }}>Категория</div>
      ),
      dataIndex: 'category',
    },
    {
      title: () => (
        <div style={{ color: `var( --primary-blue)` }}>Наименование</div>
      ),
      dataIndex: 'name',
      width: 400,
      render: (text, record, index) => (
        <div style={{ marginRight: '60px' }}>{text}</div>
      ),
    },
    {
